...
let metiluracile_salve = make "метилурациловая мазь" (7, 18) "регенерация кожи. Накладывать на ожоги, обветривания, неглубокие раны"
...
let dexametazone_ampoule = make "дексаметазон ампулы" (9, 15) "Шоки, отек головного мозга, энцефалит, множество прочего. 3-4 раза в сутки внутримышечно или под язык (по необходимости)."
...
let ketorolak_ampoule = make "кеторолак ампулы 30 мг" (12, 15) "НПВП, обезболивающее. 10-30мг до 4 раз в сутки."

let lidocaine_ampoule = make "лидокаин ампулы 2 мл 2 проц" (8, 15) "Инфильтрационная анестезия. Обколоть рану или место перелома."
...
let bifidumbacterine = make "бифидум бактерин пакетики 500 млн КОЕ" (3, 16) "Восстановление при дисбактериозе. Во время еды 1 пакетик 2 раза в сутки."

(*let eritromicine = make "эритромицин "*)

let ciproflacsacine = make "ципрофлоксацин таблетки 500 мг" (4, 16) "Антибиотик широкого спектра. 2 раза в сутки по таблетке, не менее 5 дней."

let mukaltin = make "мукалтин таблетки 500 мг" (4, 16) "Муколитик, 3 раза в день по 1 таблетке."
...
let sulfacetamid = make "сульфацил натрия, 5 мл 20 проц, капли глазные" (2, 17) "Бактериостатик. 1-2 капли 5-6 раз в сутки"
...
let aspirine = make "ацетилсалициловая кислота таблетки 500 мг" (8, 18) "Кроверазжижающее. На высоте утром по 1 табл. Противовоспалительное, жаропонижающее."
...
let asparkam = make "Аспаркам 175+175 мг" (5, 18) "Соли магния и калия. Ежедневно 1-3 таблетки 3 раза в день после еды."

let melatonin = make "Мелатонин" (1, 16) "Восстанавливает суточный цикл. За 30 минут до сна 1 таблетку, запить водой."
...
let zodak = make "Зодак" (1, 16) "Противоаллергическое. 1 табл в сутки."
