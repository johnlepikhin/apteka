...
let ibuprofen_pills = make "ибупрофен таблетки 200 мг" (9, 16) "НПВП, обезболивающее. 3-4 раза в сутки по таблетке, после еды, максимум 1200мг в сутки. Не более 5 дней"

let dexametazone_ampoule = make "дексаметазон ампулы" (9, 15) "Шоки, отек головного мозга, энцефалит, множество прочего. 3-4 раза в сутки внутримышечно или под язык."
